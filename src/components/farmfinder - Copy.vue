    ...
    <div class="containerCustom">
      ...
      <div justify-space-around class="containerCustomBody">
        <v-layout
          ...
          :style="{
            height: $store.state.options.style.farmfinder.height - 130 + 'px'
          }"
        >
            ...
            <v-layout row>
              ...
              <v-flex xs8 style="padding-left: 10px;">
                ...
                <v-btn
                  ...
                  @click="
                    showGoldClubFarm = showGoldClubFarm
                      ? false
                      : true && $store.state.CheckLogic.getGoldClubFarmlists({})
                  "
                >
          ...
          <v-layout
            ...
            wrap
            align-center
            v-show="$store.state.custom.farmfinder.showProgress"
          >
            <transition name="fade">
              ...
              >
                <v-progress-circular
                  ...
                  margin="auto"
                  >{{
                    $store.state.custom.farmfinder.progress
                  }}%</v-progress-circular
                >
              </div>
            </transition>
          ...
          <v-container
            ...
            px-0
            v-if="
              $store.state.Player.goldClubFarmlists.length > 0 &&
                showGoldClubFarm
            "
          >
            <v-radio-group
          ...
          <v-layout
            ...
            wrap
            v-if="
              $store.state.custom.FarmFinderFarms.length > 0 &&
                !showGoldClubFarm
            "
          >
            <v-flex xs12>
                ...
                <template v-slot:body="{ items }">
                  <tbody>
                    <tr v-for="(item, name, index) in items" :key="index">
                      ...
                      <td>{{ item.player.ally.name }}</td>
                      <td>{{ item.player.name }}</td>
                      <td>{{ item.name }}</td>
                      <td>{{ item.population * 1 }}</td>
                      <td>{{ item.distance.toFixed(1) }}</td>
                      <td>{{ item.player.active * 1 == 0 ? false : true }}</td>
                      <td>
                        {{
                          item.lastReport.notificationType === undefined
                            ? "/"
                            : item.lastReport.notificationType +
                              " : " +
                              item.lastReport.raidedResSum +
                              " / " +
                              item.lastReport.capacity
                        }}
                      </td>
                    </tr>
                  </tbody>
        ...
        <v-layout
          ...
          wrap
          v-show="
            $store.state.custom.FarmFinderFarms.length > 0 && !showGoldClubFarm
          "
        >
          <v-layout row wrap>
            <v-flex xs12>
